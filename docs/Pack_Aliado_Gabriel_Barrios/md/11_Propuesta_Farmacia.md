# Propuesta de valor — Farmacia (cliente B2B)

**Para:** Gabriel Barrios — Gerente de Consultoría, Grupo Morr (Valencia, Venezuela)  
**De:** Abrahan Pulido — Founder / CEO / CTO, Zonix Pharma  
**Fecha:** junio 2026

**No es una solicitud de inversión.** Resume qué le decimos al dueño o gerente de una farmacia independiente o mediana en Valencia — y qué conviene que tú valides desde consultoría local.

---

## En 60 segundos

Zonix Pharma pone a la farmacia en un **marketplace digital** sin que tenga que construir su propia app: catálogo, pedidos, recetas digitales, pagos venezolanos y entrega coordinada. La farmacia paga **cuota 45 / 60 / 70 USD + %GMV 8 / 7 / 5** según banda — no una comisión del 20–30 % como PedidosYa.

Ejemplo rápido: **3.000 USD de ventas en app al mes** (nivel Pro) → **60 + 7 % × 3.000 = 270 USD/mes** a Zonix vs **~750 USD** típico en agregador al 25%. El piloto busca **~28 farmacias activas antes del Day-D (T+90)**, **~40 en el mes 1** y **~159 al mes 12**, captadas con **4 vendedores B2B en calle**.

---

## Quién es el cliente

| Tipo | Decisor | Cómo se aborda |
|------|---------|----------------|
| Independiente, 1 sucursal | Dueño / regente | Visita 15 min en sucursal |
| Cadena pequeña (3–8 locales) | Gerente comercial o dueño | Reunión 30 min + demo |
| Cadena mediana (9–25) | Director de operaciones | Propuesta escrita + demo |
| Cadenas premium (Farmatodo, Locatel) | — | **Fuera del piloto** inicial |

Beachhead del piloto: **independientes y medianas** de Valencia metro que hoy venden por mostrador y WhatsApp, o pagan comisión alta a un agregador.

---

## Un día en la vida del dueño (hipótesis a confirmar en calle)

Antes de pitchear, Sales debe mapear el día real del dueño. Plantilla para entrevistas:

| Momento | Qué hace hoy | Dónde duele (hipótesis) |
|---------|--------------|-------------------------|
| Apertura (7–9 am) | Revisa stock, recibe droguería, abre WhatsApp | Pedidos nocturnos sin responder |
| Mediodía (12–3 pm) | Pico mostrador + WhatsApp simultáneos | Errores precio/cantidad; Rx «a ojo» |
| Cierre (6–8 pm) | Cuadre de caja, conciliar Pago Móvil | Horas conciliando; ventas no medidas |
| Fin de semana | Guardia reducida | Sin farmacéutico de turno → Rx rechazadas |

Gabriel: el ranking real de dolores debe salir de **≥5 entrevistas en Valencia**, no solo de este documento. Tipo de propuesta dominante para el one-pager: **costo total menor** vs take-rate agregador.

---

## Qué le duele hoy (hipótesis a confirmar en calle)

1. **Stock incierto para el paciente** — pierde la venta y el cliente se va a la competencia.
2. **WhatsApp sin control** — pedidos manuales, errores, sin trazabilidad.
3. **Comisión de agregador** — PedidosYa suele retener **20–30 % del GMV**; en 5.000 USD/mes son **1.000–1.500 USD** solo en comisión.
4. **Receta en papel** — validación «a ojo», sin registro si hay reclamo.
5. **Pacientes fuera del radio caminable** — no captura demanda digital de la zona.
6. **Conciliación de Pago Móvil** — horas cuadrando transferencias sin sistema único.
7. **Sin panel único** — órdenes, comprobantes y Rx repartidos entre chat, papel y Excel.

---

## Qué ofrece Zonix Pharma

### Costo predecible (modelo híbrido)

| Nivel | GMV mensual (USD, suma sucursales del RIF) | Cuota fija | % sobre GMV |
|-------|--------------------------------------------|------------|-------------|
| **Basic** | 0 – 2.000 | **45** | 8 % |
| **Pro** | 2.001 – 5.000 | **60** | 7 % |
| **Enterprise** | 5.001 en adelante | **70** | 5 % |

**Fórmula:** cuota del mes = fija del nivel + (% × GMV completado en app).

- El **envío** lo paga el paciente a la empresa de delivery — **no entra** en el GMV de la farmacia.
- Subida o bajada de nivel: tras **dos meses seguidos** en la banda superior o inferior; aviso por escrito; nueva tarifa desde el **tercer mes** (M+2).
- Farmacia nueva a mitad de mes: solo cuota fija prorrateada el primer mes parcial; desde el segundo mes, fija + %.
- Reclamos de cierre de GMV: **3 días hábiles** desde el reporte mensual oficial.

### Producto (sin construir app propia)

- App para pacientes (Android / iOS) y panel para la farmacia.
- Catálogo en ~30 minutos (archivo o carga manual).
- Validación de receta por el **farmacéutico colegiado de esa farmacia** (trazabilidad: quién, cuándo, qué).
- Pagos Venezuela nativos: Pago Móvil, transferencia, Zelle, Binance Pay — con comprobante en la orden.
- Entrega vía **empresa partner** (Zonix no tiene motos propias).
- Notificaciones al instante: nueva orden, receta validada.
- Reporte mensual de GMV que cuadra con la factura Zonix.
- Multi-sucursal bajo mismo RIF: un nivel, una factura, desglose por sucursal en panel.

### Marketing agregado

- La farmacia entra al canal de captación del marketplace (Meta, valla en piloto, flyers en aliadas).
- El costo de esa pauta lo lleva Zonix en su presupuesto operativo; la farmacia se beneficia del tráfico común.

---

## Qué NO prometemos

- No reemplazamos el ERP de la farmacia (Saint, Profit, Galac) — solo el flujo de ventas en Zonix.
- No procesamos seguros ni IVSS.
- No emitimos recetas médicas (teleconsulta).
- No somos tienda general — solo farmacia (OTC, Rx, cuidado personal afín).
- No exigimos exclusividad digital con Rappi/PedidosYa.

---

## Comparativa ilustrativa (mismo GMV 3.000 USD/mes)

| Concepto | Agregador típico VE | Zonix Pro |
|----------|---------------------|-----------|
| Comisión / cuota | ~25 % → **~750 USD** | **60 + 210 = 270 USD** |
| Orden de magnitud | — | **~64 % menos** costo de canal |

*(Mes completo estable; mes parcial de alta prorratea solo la fija.)*

---

## Discovery en calle — marco SPIN (antes de la demo)

Preguntar **hechos pasados**, no «¿usarías una app?»:

| Fase | Preguntas ejemplo | Objetivo |
|------|-------------------|----------|
| **S**ituación | ¿Cuántos pedidos digitales reciben por semana? ¿Farmacéutico en turno todos los días? | Baseline |
| **P**roblema | ¿Han perdido ventas por stock o por no aparecer en búsqueda digital? | Dolor explícito |
| **I**mplicación | Si un competidor digital captura pacientes de su zona en 6 meses, ¿qué pasa con el mostrador? | Urgencia |
| **N**ecesidad | Si tuvieran panel + demanda sin montar app propia, ¿qué KPI mejorarían primero? | Puente a demo |

---

## Cómo se vende en calle (funnel)

| Etapa | Qué pasa | Conversión orientativa |
|-------|----------|------------------------|
| Visita / flyer | Primer contacto | Base 100 % |
| Demo en tablet | App paciente + panel farmacia | ~60–75 % interés serio |
| Carta de intención | Compromiso no vinculante | ~40–55 % post-demo |
| Contrato marco | RIF, tier inicial | ~30–50 % visita → firma |
| Onboarding | Catálogo + farmacéutico | ~85–95 % en ≤14 días |
| Primera orden real | Activación | ~70–85 % en 30 días |

Motor del piloto: **4× Sales** — no solo prospección: firman, capacitan y acompañan carga de catálogo.

---

## Demo sugerida al dueño (15 minutos)

| Min | Acción |
|-----|--------|
| 0–2 | Problema: comisión alta, WhatsApp caótico, Rx sin registro |
| 2–5 | App paciente: buscar, carrito, receta, pago |
| 5–8 | Panel farmacia: orden entrante, validación Rx, entrega |
| 8–10 | Calculadora en vivo con **su** GMV estimado vs agregador |
| 10–12 | Onboarding 2 días; contrato marco anual, cuota mensual |
| 12–15 | Carta de intención + fecha visita técnica |

---

## Objeciones frecuentes (respuestas cortas)

| Objeción | Respuesta |
|----------|-----------|
| «Ya estoy en PedidosYa» | Puedes probar Zonix en paralelo; aquí pagas **45/60/70 + %**, no 20–30 % del ticket |
| «Mi farmacéutico no quiere app» | Solo ve recetas pendientes; capacitación 30 min; salida sin penalidad primeros 2 meses (contrato) |
| «¿Y si no hay pedidos?» | Marketing geo desde Day-D; objetivo escalar demanda con catálogo (**~40 M1**, curva hasta **~159 M12**) |
| «¿Es legal la Rx digital?» | Valida **tu** farmacéutico colegiado; Zonix da trazabilidad — dictamen profesional en curso |
| «¿Exclusividad con Rappi?» | No exigimos exclusividad digital |
| «¿Quién responde ante MPPS?» | La farmacia sigue siendo el establecimiento responsable; Zonix aporta logs exportables |
| «¿Cuánto tarda el onboarding?» | ~30 min catálogo + 1 h capacitación + 1 día de prueba; total ~2 días desde firma |

---

## Contrato marco — mínimos legales-operativos

Antes de órdenes reales con pacientes, cada farmacia firma documento que cubra:

1. Roles: farmacia dispensadora; Zonix como plataforma y responsable de datos de usuarios.
2. Validación Rx: farmacéutico MPPS activo; SLA acordado.
3. Recetas retenidas/controladas: pickup, identificación, libros en farmacia.
4. Pagos y comprobantes: conciliación y plazos de disputa.
5. Salida: exportación de datos y baja sin perjuicio a órdenes abiertas.

Plantillas: abogado + visado farmacéutico asesor en cláusulas de dispensación.

---

## Por qué decir sí en el piloto

1. **Costo de oportunidad:** cada mes en agregador o sin digital es margen perdido.
2. **Waiver posible:** primeras ~10 farmacias pueden negociar **2 meses sin cuota fija** (GMV se mide igual); detalle en contrato.
3. **Riesgo acotado:** salida sin penalidad acordada en los **2 primeros meses**; desde mes 3 rige el marco anual.
4. **Equipo en Carabobo:** Sales + soporte presencial, no call center remoto.

---

## Qué nos gustaría que revises, Gabriel

1. **¿45/60/70 + %GMV 8/7/5 es creíble** frente a la mentalidad del dueño valenciano?
2. **¿El ejemplo 5.000 USD/mes Pro** refleja farmacias reales de tu cartera Morr?
3. **¿Qué objeción falta** en la tabla (MPPS, droguería, competencia local)?
4. **¿Introducciones** a 3–5 farmacias piloto donde Morr tenga relación para discovery sin compromiso?

---

## Disclaimers

- **Pricing sujeto a contrato marco** firmado con abogado; cifras aquí son referencia comercial del pack.
- **GMV y tiers:** definición operativa en contrato; reclamos de cierre en 3 días hábiles desde reporte mensual.
- **Regulatorio Rx:** pendiente dictamen abogado + farmacéutico asesor antes de Day-D público.
- **Proyección ~28 / ~40 / ~159 farmacias:** plan con **4× Sales**; no garantía contractual a la farmacia individual.
- **No solicitud de inversión.**

---

**Abrahan Pulido**  
Founder / CEO / CTO — Zonix Pharma  
ing.pulido.abrahan@gmail.com · +58 412 4352014

*Zonix Pharma — junio 2026*
