# Ficha inversionista — Epakon Capital

> Ask ref.: Lean ~USD 174.102 · SAFE cap ref. USD 600.000 ([BRIEF](../../Lanzamiento/BRIEF_UNA_PAGINA.md)).  
> Forense: 21 julio 2026 · [epakon.com](https://epakon.com) (SPA + tRPC) · [AngelList](https://venture.angellist.com/epakon/syndicate) · [Ecosistema](https://ecosistemag.com/en/the-silent-architects-of-venezuelan-tech-meet-vicente-zavarce-and-diego-salas/) · SuperScout.  
> **Disclaimer:** análisis interno founder; no es asesoría legal ni financiera.

## Identificación

| Campo | Valor |
|-------|-------|
| Nombre / entidad | Epakon Capital |
| Slug carpeta | `epakon` |
| Tipo | VC pre-seed / seed (fondo + syndicate AngelList) |
| Contacto | PitchModal en [epakon.com](https://epakon.com) (`pitch.submit`) · `info@epakon.com` · LPs: `investors@epakon.com` |
| Canal de intro | Cold pitch web · warm path Casa212 / Yummy / círculo Tech Week VE |
| País / jurisdicción | Geo inversión **U.S. + LatAm**; HQ citado de forma inconsistente (LA / Boston / NY en directorios) |
| Estado relación | sin contactar |
| Feeling founder (1–10) | — |

## Ticket y términos (conocidos)

| Campo | Valor |
|-------|-------|
| Ticket esperado (USD) | **Banda creíble ~100k–300k** (AngelList); typical **~$147k**; sweet tip. **~$150k** (SuperScout). Web marketing **$50k–$1M** — **no** usar $1M como esperado |
| Instrumento | **No publicado** (SAFE / equity / convertible TBD post-pitch) |
| Cap / valuation | **No publicado** |
| Equity implícito (ref.) | Desconocido hasta offer |
| Condiciones especiales | First / early check; hands-on; red founders/YC/Harvard; **sin** residencia obligatoria; HealthTech en form de pitch |
| Fuentes | epakon.com · AngelList syndicate · SuperScout · Ecosistema · bundle JS / tRPC forense |

### Matriz vs ask Zonix

| Dimensión | Pack Zonix | Epakon | Gap |
|-----------|------------|--------|-----|
| Ticket | Lean ~174k | ~100–300k (tip. ~147–150k) | Compatible; puede cubrir Lean o parte |
| Dilución | SAFE cap 600k | **Opaca** | Pedir instrumento + cap/% por escrito |
| Vehículo | Ideal Delaware | US/LatAm founders; path US habitual en portfolio | Alineado en práctica |
| Tiempo | — | Pitch inmediato; cierre VC típico (semanas) | Sin mudanza 12 sem |
| Probabilidad | — | Deal-by-deal (no cupos fijos públicos) | Mejor que lottery 6/5 cupos |

## Lo bueno

- Ticket en banda que **cubre** el ask Lean (~174k) si escriben ~150–300k.
- **Misma red VE** que Casa212 (Diego Salas + Vicente Zavarce / Yummy) **sin** exigir 12 semanas Caracas.
- Thesis diaspora + LatAm; form incluye **HealthTech**; portfolio con Cashea, Yummy, Meru, HeroUI, Lime Therapeutics (health).
- Canal founder claro: PitchModal + deck PDF/PPTX o link.
- Beneficios portfolio (AWS / Lazo / Vouch / Polsinelli) si cierran.
- Apply/pitch **paralelo** a Casa212 / 500 hasta aceptar exclusividad en otro lado.

## Lo malo / riesgos

- Términos (**instrumento, cap, %**) **no públicos** → score S bajo hasta offer.
- Web dice **$50k–$1M**; AngelList **$100–300k** → no anclar runway al techo de $1M.
- Claim **40+** companies vs **27** con URL en el sitio → inventario incompleto / marketing.
- Pharma/Rx VE no es el deal típico (más fintech/AI/consumer); hay que vender HealthTech + marketplace.
- Insights (`insights.epakon.com`) es portal **LP**, no founder.
- Solape con Casa212: misma órbita; no es “otro cheque independiente” de red.

## Scoring

| Código | Criterio | Peso máx. | Puntos | Notas |
|--------|----------|-----------|--------|-------|
| T | Ajuste ticket vs ask | 20 | **17** | ~150–300k cubre Lean; tip. ~147k cerca; no asumir $1M |
| S | Términos / dilución | 20 | **8** | Instrumento/cap/% opacos |
| E | Valor estratégico | 15 | **14** | Red VE + Yummy/Cashea + HealthTech + puente US |
| V | Velocidad de cierre | 15 | **12** | Pitch web ya; cierre desconocido pero sin gate residencia |
| C | Alineación / control | 15 | **12** | Early check / hands-on; sin 12 sem Caracas; board no publicado |
| R | Riesgo reputacional / regulatorio | 15 | **10** | Ops VE + OFAC mismo cuidado pack; HQ inconsistente menor |
| | **TOTAL** | **100** | **73** | |

**% de conveniencia:** **73**  
**Veredicto:** **nurture** (borde priorizar) — pitch cuando deck esté listo; confirmar banda de cheque + términos por escrito

## Próximos pasos

> One-pager: [RESUMEN_CEO.md](RESUMEN_CEO.md) / [PDF](RESUMEN_CEO.pdf).

1. Preparar deck Zonix Pharma (HealthTech / marketplace VE) + elevator 2–3 frases.
2. Decidir cold PitchModal vs warm intro (Casa212 / Yummy).
3. En conversación: pedir **instrumento**, **monto**, **cap/%**, timeline de cierre.
4. Paralelo: Casa212 (si hay Caracas) / 500 LatAm / ALGEN.

## Enlaces

- Resumen CEO: [RESUMEN_CEO.md](RESUMEN_CEO.md)
- NOTAS: [NOTAS.md](NOTAS.md)
- Sitio: https://epakon.com
- AngelList: https://venture.angellist.com/epakon/syndicate
- Briefing: [`../../Lanzamiento/BRIEFING_INVERSORES_VE_2026.md`](../../Lanzamiento/BRIEFING_INVERSORES_VE_2026.md)
